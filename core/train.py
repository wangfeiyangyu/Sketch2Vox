...
def train_net(cfg):
    ...
    #print('[DEBUG] %s Parameters in Transformer_vgg: %d.' % (dt.now(), utils.network_utils.count_parameters(transformer)))
    ...
    #transformer.apply(utils.network_utils.init_weights)
    ...
    if cfg.TRAIN.POLICY == 'adam':
        ...
        #transformer_solver = torch.optim.Adam(transformer.parameters(), lr=cfg.TRAIN.ENCODER_LEARNING_RATE, betas=cfg.TRAIN.BETAS)
        fusion_solver = torch.optim.Adam(decoder.parameters(), lr=cfg.TRAIN.DECODER_LEARNING_RATE, betas=cfg.TRAIN.BETAS)
        ...
        #transformer_solver = torch.optim.SGD(transformer.parameters(), lr=cfg.TRAIN.ENCODER_LEARNING_RATE, momentum=cfg.TRAIN.MOMENTUM)
        fusion_solver = torch.optim.SGD(fusion.parameters(), lr=cfg.TRAIN.DECODER_LEARNING_RATE, momentum=cfg.TRAIN.MOMENTUM)
    ...
    #transformer_vgg_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(transformer_solver, milestones=cfg.TRAIN.MERGER_LR_MILESTONES, gamma=cfg.TRAIN.GAMMA)
    ...
    if torch.cuda.is_available():
        ...
        #transformer_vgg = torch.nn.DataParallel(transformer).cuda()
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        ...
        with tqdm(train_data_loader, desc='train network') as train_data_loader:
            for batch_idx, (taxonomy_ids, taxonomy_names, sample_names, rendering_images, ground_truth_volumes, image_points) in enumerate(train_data_loader):
                ...
                # view:重构不同维度的张量 contiguous:确保Tensor底层一维数组元素的存储顺序与Tensor按行优先一维展开的元素顺序一致
                image_features = image_features.view(batch_size, channels, f_h, f_w).contiguous()
                # torch中一些操作会改变原数据，比如：narrow()
                # view()
                # expand()
                # transpose()
                # 等操作，在使用transpose()
                # 进行转置操作时，pytorch并不会创建新的、转置后的tensor，而是修改了tensor中的一些属性（也就是元数据），使得此时的offset和stride是与转置tensor
                # 相对应的。转置的tensor和原tensor的内存是共享的，即改变转置后的tensor, 原先tensor中内容也会改变，而contiguous方法就类似深拷贝，使得上面这些操作不会改变元数据
        ...
        if iou > best_iou:
            ...
            utils.network_utils.save_checkpoints(cfg=cfg, file_path=ckpt_file, epoch_idx=epoch_idx + 1, best_epoch=best_epoch, best_iou=best_iou,
                                                 ...
                                                 #transformer=transformer, transformer_solver=transformer_solver,
