...
from core.LMNet.test_LMNet import test_net
...
def train_net(cfg):
    ...
    train_transforms = utils.data_transforms.Compose([
        ...
        #utils.data_transforms.RandomRotate(),
...
# Set up data loader
    ...
    print('[DEBUG] %s Parameters in Encoder3D: %d.' % (dt.now(), utils.network_utils.count_parameters(encoder3D)))
    ...
    encoder3D.apply(utils.network_utils.init_weights)
    ...
    if cfg.TRAIN.POLICY == 'adam':
        ...
        encoder3D_solver = torch.optim.Adam(encoder3D.parameters(), lr=cfg.TRAIN.ENCODER_LEARNING_RATE, betas=cfg.TRAIN.BETAS)
        ...
        encoder3D_solver = torch.optim.SGD(encoder3D.parameters(), lr=cfg.TRAIN.DECODER_LEARNING_RATE, momentum=cfg.TRAIN.MOMENTUM)
    ...
    encoder3D_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(encoder3D_solver, milestones=cfg.TRAIN.ENCODER_LR_MILESTONES, gamma=cfg.TRAIN.GAMMA)
    ...
    if torch.cuda.is_available():
        ...
        encoder3D = torch.nn.DataParallel(encoder3D).cuda()
    ...
    # Training encoder3D-decoder loop
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES_AUTOENCODER):
        # Tick / tock
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images, ground_truth_volumes, image_points) in enumerate(train_data_loader):
            ...
            # Train the encoder3D, decoder, refiner
            volumn_vectors = encoder3D(ground_truth_volumes)
            generated_volumes = decoder(volumn_vectors)
            ...
            batch_end_time = time()
            print('[Train reconstruction loss] %s [Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) reconstruction_loss = %.4f'
                % (dt.now(), epoch_idx + 1, cfg.TRAIN.NUM_EPOCHES_REGRESS, batch_idx + 1, n_batches, batch_time.val, data_time.val, refiner_loss.item()))'''
        ...
        # Tick / tock
        epoch_end_time = time()
        print('[Train reconstruction loss] %s Epoch [%d/%d] EpochTime = %.3f (s) reconstruction_loss = %.4f'
              % (dt.now(), epoch_idx + 1, cfg.TRAIN.NUM_EPOCHES_AUTOENCODER, epoch_end_time - epoch_start_time, refiner_losses.avg))
        # Append epoch loss to TensorBoard
        train_writer.add_scalar('3D_Encoder/reconstruction_loss', refiner_losses.avg, epoch_idx + 1)

    # Training encoder-encoder3D loop
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES_REGRESS):
        # Tick / tock
        epoch_start_time = time()
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images, ground_truth_volumes, image_points) in enumerate(train_data_loader):
            ...
            volumn_features = encoder3D(ground_truth_volumes)
            encoder_loss = torch.mean(euclidean_loss(image_features, volumn_features))
            ...
            batch_time.update(time() - batch_end_time)
            batch_end_time = time()
            print('[Train regression loss] %s [Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) EDLoss = %.4f'
                % (dt.now(), epoch_idx + 1, cfg.TRAIN.NUM_EPOCHES_REGRESS, batch_idx + 1, n_batches, batch_time.val, data_time.val, encoder_loss.item()))'''
        ...
        # Tick / tock
        epoch_end_time = time()
        print('[Train regression loss] %s Epoch [%d/%d] EpochTime = %.3f (s) regression_loss = %.4f'
              %(dt.now(), epoch_idx + 1, cfg.TRAIN.NUM_EPOCHES_REGRESS, epoch_end_time - epoch_start_time, encoder_losses.avg))
        # Append epoch loss to TensorBoard
        train_writer.add_scalar('Encoder/regression_loss', encoder_losses.avg, epoch_idx + 1)

        '''# Validate the training models
        iou = test_net(cfg, epoch_idx + 1, output_dir, val_data_loader, val_writer, pointNetEncoder, decoder, refiner)
        ...
        if iou > best_iou:
            ...
            utils.network_utils.save_checkpoints(cfg=cfg, file_path=os.path.join(ckpt_dir, 'best-ckpt.pth'),epoch_idx=epoch_idx + 1, encoder=encoder,
                                                 encoder_solver=encoder_solver, decoder=decoder,decoder_solver=decoder_solver,
                                                 refiner=refiner, refiner_solver=refiner_solver, best_iou=best_iou,best_epoch=best_epoch)'''
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES_FINETUNE):
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images, ground_truth_volumes, image_points) in enumerate(train_data_loader):
            ...
            # Train the encoder, encoder3D, decoder, refiner
            image_features = encoder(rendering_images).squeeze(dim=1)
            point_features = pointNetEncoder(image_points)
            volumn_vectors = encoder3D(ground_truth_volumes)
            image_features = merger(torch.stack([image_features, point_features], dim=2))
            encoder_loss = torch.mean(euclidean_loss(image_features, volumn_vectors))
        ...
        # Validate the training models
        iou = test_net(cfg, epoch_idx + 1, output_dir, val_data_loader, val_writer, encoder, pointNetEncoder, merger, decoder, refiner)
