...
def val_net(cfg, epoch_idx, writer, data_loader, pointNetEncoder, decoder, refiner):
    ...
    with tqdm(data_loader, desc='validate network') as data_loader:
        for sample_idx, (taxonomy_ids, taxonomy_names, sample_names, rendering_images, ground_truth_volumes, image_points) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                raw_features, generated_volumes = decoder(point_features)
                ...
                Fpos_ce = bce_loss(generated_volumes[unoccupied_voxels], ground_truth_volumes[unoccupied_voxels])
                Fneg_ce = bce_loss(generated_volumes[occupied_voxels], ground_truth_volumes[occupied_voxels])
                encoder_loss = (torch.pow(Fpos_ce, 2) + torch.pow(Fneg_ce, 2))

                generated_volume = refiner(generated_volumes)
                # 计算refiner_loss
                unoccupied_voxels = torch.eq(ground_truth_volumes, 0)
                occupied_voxels = torch.eq(ground_truth_volumes, 1)
                Fpos_ce = bce_loss(generated_volume[unoccupied_voxels], ground_truth_volumes[unoccupied_voxels])
                Fneg_ce = bce_loss(generated_volume[occupied_voxels], ground_truth_volumes[occupied_voxels])
                ...
                for th in cfg.TEST.VOXEL_THRESH:
                    ...
                    union = torch.sum(torch.ge(_volume.add(ground_truth_volumes), 1)).float()
