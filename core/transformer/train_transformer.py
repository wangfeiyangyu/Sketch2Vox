...
from core.transformer.test_transformer import test_net
...
def train_net(cfg):
    ...
    print('[DEBUG] %s Parameters in Transformer: %d.' % (dt.now(), utils.network_utils.count_parameters(transformer)))
    ...
    if cfg.TRAIN.POLICY == 'adam':
        ...
        transformer_solver = torch.optim.Adam(transformer.parameters(), lr=cfg.TRAIN.DECODER_LEARNING_RATE, betas=cfg.TRAIN.BETAS)
        ...
        transformer_solver = torch.optim.SGD(transformer.parameters(), lr=cfg.TRAIN.DECODER_LEARNING_RATE, momentum=cfg.TRAIN.MOMENTUM)
    ...
    transformer_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(transformer_solver, milestones=cfg.TRAIN.ENCODER_LR_MILESTONES, gamma=cfg.TRAIN.GAMMA)
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES_FINETUNE):
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images, ground_truth_volumes, image_points) in enumerate(train_data_loader):
            ...
            # Train the encoder, pointNetEncoder, decoder, refiner
            ...
            image_features = transformer(image_features)  # 使用self-attention进行特征融合
        ...
        iou = test_net(cfg, epoch_idx + 1, output_dir, val_data_loader, val_writer, encoder, transformer, decoder, refiner)
