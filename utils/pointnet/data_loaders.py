...
class ShapeNetDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        taxonomy_name, sample_name, rendering_images, volume, points = self.get_datum(idx)

        if self.transforms:
            rendering_images = self.transforms(rendering_images)

        return taxonomy_name, sample_name, rendering_images, volume, points
    ...
    def get_datum(self, idx):
        ...
        return taxonomy_name, sample_name, np.asarray(rendering_images), volume, points
...
class ShapeNetDataLoader:
    ...
    def get_files_of_taxonomy(self, taxonomy_folder_name, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            point_path = self.point_path % (taxonomy_folder_name, sample_name)
            if not os.path.exists(point_path):
                print('[WARN] %s Ignore sample %s/%s since point file not exists.' % (dt.now(), taxonomy_folder_name, sample_name))
                continue
