...
class Normalize(object):
    ...
    def __call__(self, rendering_images):
        assert (isinstance(rendering_images, np.ndarray))
        img_channels = rendering_images[0].shape[2]
        '''if img_channels > 3:
            rendering_images = rendering_images[..., 0:3]'''
...
class Resize(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        img_size_c = rendering_images[0].shape[2]
        processed_images = np.empty(shape=(0, self.img_size_h, self.img_size_w, img_size_c))

        for img_idx, img in enumerate(rendering_images):
            img_height, img_width, _ = img.shape
            processed_image = cv2.resize(img, (self.img_size_w, self.img_size_h))
            processed_images = np.append(processed_images, [processed_image], axis=0)
        return processed_images[..., 0:3]
...
class RandomRotate(object):
    def __call__(self, rendering_images):
        """ Randomly rotate the images to augument the dataset
            rotation is different per sample based along up direction
            Input:
              VxHxWXC array, original batch of images
            Return:
              VxHxWXC array, rotated batch of images
        """
        if len(rendering_images) == 0:
            return rendering_images
        ...
        rotated_data = np.zeros(rendering_images.shape, dtype=np.float32)
        for k in range(rendering_images.shape[0]):
            ...
            shape_pc = rendering_images[k, ...]
            rotation_angle = np.random.uniform() * 360
            rotate_matrix = cv2.getRotationMatrix2D((img_width/2, img_height/2), rotation_angle, 1.0)
            shape_pc = cv2.warpAffine(src=shape_pc, M=rotate_matrix, dsize=(img_width, img_height), borderValue=(1.0, 1.0, 1.0))
...
class Binarize(object):
    def __init__(self, threshold):
        """Set the threshold for image binary"""
        self.threshold = threshold

    def __call__(self, rendering_images):
        rendering_images = np.where(1.0-rendering_images > self.threshold, 1.0, 0)
        return rendering_images
