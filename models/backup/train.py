...
def train_net(cfg):
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images,
            ...
            raw_features, generated_volumes = decoder(Z_vae)

            if cfg.NETWORK.USE_MERGER and epoch_idx >= cfg.TRAIN.EPOCH_START_USE_MERGER:
                generated_volumes = merger(raw_features, generated_volumes)
            else:
                generated_volumes = torch.mean(generated_volumes, dim=1)
            KLLoss = -0.5 * torch.sum(1 + img_logVars - torch.pow(img_Zmeans, 2) - torch.exp(img_logVars))  # 计算生成向量Z与标准正态分布的相对熵
            encoder_loss = (bce_loss(generated_volumes, ground_truth_volumes) + KLLoss) * 10
