...
def test_net(cfg,
    ...
    for sample_idx, (taxonomy_id, sample_name, rendering_images, ground_truth_volume) in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            img_Zmeans, img_logVars = encoder(rendering_images)
            Z_vae = encoder.module.reparameterize(img_Zmeans, img_logVars)
            raw_features, generated_volume = decoder(Z_vae)
            ...
            if output_dir and sample_idx < 3:
                ...
                test_writer.add_image('Test Sample#%02d/Volume Reconstructed' % sample_idx, rendering_views,
                                      epoch_idx, dataformats='HWC')
                gtv = ground_truth_volume.cpu().numpy()
                rendering_views = utils.binvox_visualization.get_volume_views(gtv, os.path.join(img_dir, 'test'), epoch_idx)
                test_writer.add_image('Test Sample#%02d/Volume GroundTruth' % sample_idx, rendering_views,
                                      epoch_idx, dataformats='HWC')

                binvox_dir = output_dir % 'binvoxs'
                if not os.path.exists(binvox_dir):
                    os.makedirs(binvox_dir)
                with open(os.path.join(binvox_dir, '%s-%s.binvox' % (taxonomy_id, sample_name)), 'wb') as f:
                    gv = (gv.squeeze() > 0.5).astype(np.int32)  # change numpy datatype to bool
                    m1 = utils.binvox_rw.Voxels(data=gv, dims=gv.shape, translate=[0, 0, 0], scale=1, axis_order='xyz')
                    m1.write(f)
