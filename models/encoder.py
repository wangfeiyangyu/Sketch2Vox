...
# from models.tps.STN import STN_net
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, rendering_images):
        ...
        for img in rendering_images:
            ...
            # print(features.size())    # torch.Size([batch_size, 512, 28, 28])
            # features = self.STN_model(features)
            # print(features.size())    # torch.Size([batch_size, 512, 28, 28])
            ...
            # print(features.size())    # torch.Size([batch_size, 16384])
            features = self.layer4(features)
            # print(features.size())    # torch.Size([batch_size, 4096])
            features = self.layer5(features)
            # print(features.size())    # torch.Size([batch_size, 4096])
            features = self.layer6(features)
            # print(features.size())    # torch.Size([batch_size, 64])
            image_features.append(features)

        image_features = torch.stack(image_features).permute(1, 0, 2).contiguous()
        # print(image_features.size())  # torch.Size([batch_size, n_views, 64])
